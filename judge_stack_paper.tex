...
% ══════════════════════════════════════════════════════════════════════════════
\begin{center}
    {\Huge \textbf{\color{primary} Thalir: Non-Destructive Foliar Ionome Sensing \& Grounded RAG}} \\[6pt]
    {\large \textbf{A Complete Cyber-Physical Precision Agriculture Platform for Smallholder Farmers}} \\[4pt]
    {\small \textbf{Project Thalir (தளிர்) — Venture Hack Initiative} $\cdot$ Real-Time Dual-Wavelength Optical Chemometrics \& TNAU Expert Advisory}
\end{center}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{n8n + pgvector Grounded RAG}: Automated workflow orchestrated in \textbf{n8n} utilizing \textbf{PostgreSQL with pgvector} for semantic embeddings and hybrid filtering over 1,338 TNAU records, eliminating AI hallucination by retrieving exact government-certified dosages (soil urea kg/ha, foliar DAP \%, CaCl$_2$ sprays, split intervals).
...
\begin{tikzpicture}[
    ...
    \node[block, left=1.2cm of tnau] (rag) {\textbf{n8n + pgvector}\\Cosine Similarity Query\\Hybrid Crop Filter};
    \node[block, left=1.2cm of rag] (presc) {\textbf{Clinical Prescription}\\Grounded TNAU Dosages\\Soil kg/ha \& Foliar \%};
...
\begin{minipage}[t]{0.48\textwidth}
\begin{tcolorbox}[colback=white,colframe=primary,title=\textbf{n8n Workflow \& pgvector Retrieval Architecture},arc=2mm]
\textbf{How We Eliminate AI Hallucinations via n8n + pgvector}:
Standard LLMs hallucinate agrochemical dosages, risking severe foliar chemical burns. Our pipeline uses an event-driven \textbf{n8n workflow} connected to \textbf{PostgreSQL pgvector}:
\begin{enumerate}[leftmargin=*]
    \item \textbf{Event Ingestion (n8n)}: n8n listens to MQTT events or webhooks from the edge and triggers upon detecting a deficient nutrient ($\tau_{\text{crit}}$).
    \item \textbf{Vector Similarity (pgvector)}: Embeddings of all 1,338 TNAU records are indexed in PostgreSQL using \texttt{pgvector} (HNSW/IVFFlat cosine distance). n8n executes a hybrid query:
    \begin{equation*}
        \min_{\mathbf{v}} \|\mathbf{v}_{\text{query}} - \mathbf{v}_{\text{doc}}\|^2 \quad \text{s.t. } \text{crop} = \text{target}
    \end{equation*}
    \item \textbf{Grounded Prompt Injection}: n8n binds the retrieved TNAU records directly into the LLM context, forbidding ungrounded dosages.
    \item \textbf{Output Delivery}: Formulates exact soil split timing (kg/ha) and foliar dilution (\% and g/L) and pushes directly to the mobile dashboard.
\end{enumerate}
\end{tcolorbox}
\end{minipage}
\hfill
