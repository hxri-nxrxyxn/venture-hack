...
class MQTTClient:
    ...
    def connect(self, clean_session=True, timeout=None):
        ...
        if self.ssl is True:
            ...
            sni = self.ssl_params.get("server_hostname", self.server) if isinstance(self.ssl_params, dict) else self.server
            self.sock = self.ssl.wrap_socket(self.sock, server_hostname=sni)
        if timeout:
            try:
                self.sock.settimeout(timeout)
            except:
